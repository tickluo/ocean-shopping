<template>
  <div>
    <article class="base_info">
      <input type="text" class="buyer_name" placeholder="收件人">
      <div class="address_tips">
        收件人姓名需与身份证姓名一致，否则影响清关
      </div>
      <input type="text" class="buyer_name" placeholder="联系电话">
      ...
      <input type="text" class="buyer_name" placeholder="身份证号">
    ...
    <article class="base_info">
      <div class="selct_area" @click="pickRegion">
        <input type="text" id="regionPicker" readonly="true" class="buyer_name" placeholder="选择所在区域">
        <img :src="images.iconR" alt="" class="icon_right">
      </div>
      <textarea class="buyer_name no_top_bor" placeholder="详细地址..."></textarea>
      <input type="text" id="zipNumber" v-el:zip class="buyer_name no_top_bor" placeholder="邮政编码">
    </article>

    <article class="base_info">
      <div class="sel_other_company area_live">
    ...
    <footer class="shopping_footer">
      <div class="icon_shopping_cart_1" v-link="{name:'submitShopping'}">
      ...
      <div class="into_cart_btn"><img class="icon_into_shopping_cart" :src="images.iconOk"/> <span
        class="dis_inline_block">保存收货地址</span></div>
    </footer>
  </div>
</template>
<style>
...
<script>
  ...
  export default{
    ...
    methods: {
      ...
      uploadImg (type) {
        let context = this
        lrz(context.$event.target.files[0], {})
          .then(res => {
            if (type === 'positive') this.positiveImg = res.base64
            else this.oppositeImg = res.base64
          })
      },
      deleteImg (type) {
        if (type === 'positive') this.positiveImg = ''
        else this.oppositeImg = ''
      }
    }
  }
</script>
